...
export default defineConfig(({ mode }) => {
  ...
  return {
    base: process.env.NODE_ENV === "production" ? "/techno-injaz-front/" : "/",
    build: {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ["**/.figma/**"] },
      // The app is served through the Figma preview proxy over HTTPS, so the
      // HMR client must reconnect through that same host over wss/443 instead
      // of the default localhost:<port> websocket.
      hmr: {
        protocol: "wss",
        clientPort: 443,
      },
